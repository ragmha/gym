...
| Visual tests | Playwright + Chromium | Static export served via `serve@14` |
| Lint/format | ESLint 9 (flat config) + Prettier | `eslint --fix` runs on staged files |
| Hooks | Husky + lint-staged | Pre-commit auto-formats |

Platform priority: **iOS first**, **web first-class for visual tests**, **Android best-effort**.
...
| `bun run test:visual:web` | Playwright visual screenshots (auto-builds `dist/`) |
| `bun run build:web` | Static export → `dist/` |
...
| **visual screenshots** | Frontend PRs | Playwright captures Expo Web routes, posts artifacts |
