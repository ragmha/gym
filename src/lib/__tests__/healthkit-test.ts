...
describe('healthkit service', () => {
  ...
  describe('initializeHealthKit', () => {
    it('resolves true on successful iOS initialization', async () => {
      mockHealthKit.initHealthKit.mockImplementation(
        (_perms: any, callback: (error: string | null) => void) => {
          callback(null)
        },
      )
  ...
  describe('getDailySteps', () => {
    it('returns step count on successful iOS request', async () => {
      mockHealthKit.getStepCount.mockImplementation(
        (
          _opts: any,
          callback: (err: string | null, results: { value: number }) => void,
        ) => {
    ...
    it('returns 0 on iOS error', async () => {
      const consoleWarnSpy = jest.spyOn(console, 'warn').mockImplementation()
      mockHealthKit.getStepCount.mockImplementation(
        (_opts: any, callback: (err: string | null) => void) => {
    ...
    it('handles null/undefined results gracefully', async () => {
      mockHealthKit.getStepCount.mockImplementation(
        (_opts: any, callback: (err: string | null, results: any) => void) => {
          callback(null, null)
        },
      )
  ...
  describe('getDailyCalories', () => {
    ...
    it('handles empty results array', async () => {
      mockHealthKit.getActiveEnergyBurned.mockImplementation(
        (
          _opts: any,
          callback: (err: string | null, results: any[]) => void,
        ) => {
          callback(null, [])
        },
      )
  ...
  describe('getRecentWorkouts', () => {
    it('returns workouts on successful iOS request', async () => {
      ...
      mockHealthKit.getSamples.mockImplementation(
        (
          _opts: any,
          callback: (err: string | null, results: any[]) => void,
        ) => {
    ...
    it('uses default of 7 days when no parameter provided', async () => {
      mockHealthKit.getSamples.mockImplementation(
        (opts: any, callback: (err: string | null, results: any[]) => void) => {
